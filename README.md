...
An interactive 3D visualization of NASA's Artemis II free-return lunar flyby trajectory, built with Three.js and calibrated against public NASA mission data and milestones from the April 1–10, 2026 mission.

🔗 **[Live Demo →](https://geonet-myanmar.github.io/artemis2-trajectory/)**

![Artemis II Trajectory Animation](artemis2-trajectory.gif)

The bundled GIF is captured from the current `index.html` renderer so the preview animation matches the interactive visualization.
...
- **Calibrated trajectory** — Stylized figure-eight free-return path aligned to NASA's published mission profile, including the HEO spiral, outbound coast, Moon-relative far-side flyby, and separate return arc
...
├── artemis2-trajectory.gif    # Animated GIF captured from the current HTML renderer (1200×800, 200 frames)
...
| Trajectory | Parametric free-return path calibrated to NASA mission phases, with lunar flyby computed in the Moon's moving frame |
| GIF asset | Captured from the current `index.html` renderer with headless Edge and ImageMagick |
